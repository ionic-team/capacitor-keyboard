...
public class KeyboardHeightFilterTest {
    ...
    @Test
    public void testDynamicInputContextCaching() {
        KeyboardHeightFilter filter = new KeyboardHeightFilter();
        filter.setEventMode(Keyboard.EventMode.LAST_KNOWN);

        // 1. Initial stable open for TEXT input (316)
        filter.onPrepareAnimation();
        filter.filterOnStart(true, 316, "1080x1920|swiftkey|text");
        filter.filterOnEnd(true, 316, "1080x1920|swiftkey|text");

        // 2. Keyboard hides
        filter.onPrepareAnimation();
        filter.filterOnEnd(false, 0, "1080x1920|swiftkey|text");

        // 3. User focuses NUMERIC input (272)
        // Since the context is 'numeric', this gets an isolated cache bucket!
        filter.onPrepareAnimation();
        KeyboardHeightFilter.FilterResult startResult = filter.filterOnStart(true, 272, "1080x1920|swiftkey|numeric");

        // Assert: It MUST emit 272 and NOT clamp upwards to 316!
        assertEquals(272, startResult.emitHeight);
    }

    @Test
    public void testConfirmPendingHeightForTypeSwitch() {
        KeyboardHeightFilter filter = new KeyboardHeightFilter();
        filter.setEventMode(Keyboard.EventMode.LAST_KNOWN);

        // 1. Stable TEXT input (316)
        filter.onPrepareAnimation();
        filter.filterOnStart(true, 316, "1080x1920|swiftkey");
        filter.filterOnEnd(true, 316, "1080x1920|swiftkey");

        // Consume the immediate post-animation static layout pass (which is normal OS behavior)
        filter.filterOnApplyWindowInsets(true, 316, "1080x1920|swiftkey");

        // 2. User manually dismisses the suggestion bar, OR developer didn't use setInputContext
        // and switched inputs. OS resizes instantly (no animation) to 272.
        KeyboardHeightFilter.FilterResult staticResult = filter.filterOnApplyWindowInsets(true, 272, "1080x1920|swiftkey");

        // Because the cache has 316, it aggressively clamps it upward to 316!
        // But since it clamped, it flags it as pending and suppresses emission.
        assertFalse(staticResult.shouldEmit);
        assertEquals(272, staticResult.pendingHeight);

        // 3. The 0ms delay Runnable fires and confirms the height is STILL 272.
        KeyboardHeightFilter.FilterResult confirmResult = filter.confirmPendingHeight(true, 272, "1080x1920|swiftkey");

        // Assert: It MUST confirm the 272 height, overwrite the 316 cache, and emit it!
        assertTrue(confirmResult.shouldEmit);
        assertEquals(272, confirmResult.emitHeight);
    }

    @Test
    public void testReevaluateAfterInputContextChange() {
        KeyboardHeightFilter filter = new KeyboardHeightFilter();
        filter.setEventMode(Keyboard.EventMode.LAST_KNOWN);

        // 1. Stable TEXT input (316) and stable NUMERIC input (272) both cached
        filter.onPrepareAnimation();
        filter.filterOnStart(true, 316, "1080x1920|swiftkey|text");
        filter.filterOnEnd(true, 316, "1080x1920|swiftkey|text");
        filter.onPrepareAnimation();
        filter.filterOnStart(true, 272, "1080x1920|swiftkey|numeric");
        filter.filterOnEnd(true, 272, "1080x1920|swiftkey|numeric");
        filter.onPrepareAnimation();
        filter.filterOnEnd(false, 0, "1080x1920|swiftkey|numeric");

        // 2. User focuses TEXT input, but the asynchronous JS message 'setInputContext("text")' is slow.
        // OS starts animation. Java defaults to previous context ("numeric").
        filter.onPrepareAnimation();
        KeyboardHeightFilter.FilterResult startResult = filter.filterOnStart(true, 316, "1080x1920|swiftkey|numeric");

        // Because it thinks it's a numeric keyboard, it erroneously clamps the 316 animation start to 272!
        assertEquals(272, startResult.emitHeight);

        // 3. Mid-animation, the 'setInputContext("text")' message arrives!
        KeyboardHeightFilter.FilterResult reevalResult = filter.filterOnInputContextChanged(true, 316, "1080x1920|swiftkey|text");

        // Assert: It MUST recalculate using the "text" bucket and fire a corrective keyboardWillShow(316)!
        assertTrue(reevalResult.shouldEmit);
        assertEquals(316, reevalResult.emitHeight);
    }
}
